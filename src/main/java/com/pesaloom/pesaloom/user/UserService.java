...
import com.pesaloom.pesaloom.exception.EmailAlreadyInUseException;
...
public class UserService {
    ...
    @Transactional
    public User createShellUser(String firstName, String lastName) {
        String email = "borrower+" + UUID.randomUUID() + "@pesaloom.internal";
        String unusablePassword = passwordEncoder.encode(UUID.randomUUID().toString());
        User user = new User(email, unusablePassword, firstName.trim(), lastName.trim());
        user.disable();
        return userRepository.save(user);
    }

    public List<User> findAllByRole(Role role) {
        return userRepository.findAllByRoleOrderByFirstNameAsc(role);
    }
}
